...
export const sendBookingConfirmationEmail = async (bookingId: string) => {
  if (!resend) {
    console.log("[Email Service] Resend API key not configured. Skipping email send for booking:", bookingId);
    return;
  }
  ...
  try {
    ...
    const emailHtml = `
      <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #F9F6F0; padding: 40px; border: 2px solid #D4AF37;">
        <div style="text-align: center; margin-bottom: 30px;">
          ...
          <p style="color: #D4AF37; margin: 5px 0 0; font-size: 12px; letter-spacing: 2px; text-transform: uppercase;">Booking Confirmed</p>
        </div>
        ...
        <div style="background-color: white; padding: 30px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
          ...
          <p style="color: #555; line-height: 1.6;">Your reservation for <strong>${eventName}</strong> has been successfully confirmed. We are thrilled to host you for an authentic South Indian culinary experience.</p>
          ...
          <table style="width: 100%; margin-top: 25px; border-collapse: collapse;">
            ...
            <tr>
              <td style="padding: 12px 0; color: #777; font-size: 12px; text-transform: uppercase; font-weight: bold;">Amount Paid</td>
        ...
        <div style="text-align: center; margin-top: 30px; color: #777; font-size: 12px;">
          <p>Please present this email or your E-Ticket at the entrance.</p>
    ...
    await resend.emails.send({
      ...
      to: [userEmail],
      subject: `Your Booking is Confirmed: ${eventName}`,
      html: emailHtml,
    });

    console.log(`[Email Service] Confirmation email sent successfully to ${userEmail}`);
  } catch (error) {
